...
from app.synth.preprocess import TableEncoder, _column_fingerprint, stable_seed
...
class RelationalSPN:
    ...
    def __init__(self) -> None:
        ...
        # table -> {duplicate column: the column it copies}. The encoder mirrors
        # duplicates among the columns it models, but keys never reach it (they
        # are minted structurally), so a key and its copy would otherwise be
        # generated by two unrelated mechanisms and disagree. Recorded over the
        # WHOLE frame and re-applied after sampling, so the copy survives
        # whichever side of that split each column fell on.
        self.column_mirrors: dict[str, dict[str, str]] = {}
    ...
    def fit(
        ...
        for position, table_name in enumerate(model.order):
            ...
            # Record duplicate columns across the whole frame, keys included.
            # First occurrence in column order wins, so the copy always names a
            # column that is already built by the time the copy is applied.
            seen_values: dict[str, str] = {}
            mirrors: dict[str, str] = {}
            for column in frame.columns:
                fingerprint = _column_fingerprint(frame[column])
                if fingerprint is None:
                    continue
                if fingerprint in seen_values:
                    mirrors[column] = seen_values[fingerprint]
                else:
                    seen_values[fingerprint] = column
            if mirrors:
                model.column_mirrors[table_name] = mirrors
    ...
    def sample(
        ...
        for position, table_name in enumerate(self.order):
            ...
            if not parent_fks:
                ...
                frame = self._apply_column_mirrors(table_name, frame)
    ...
    def _apply_column_mirrors(
        self, table_name: str, frame: pd.DataFrame
    ) -> pd.DataFrame:
        """Re-copy columns that were exact duplicates in the source.

        Applied after the primary key is minted, so a key and its copy agree.
        For a pair the encoder already mirrored this is a no-op; for a pair
        split across the key/modelled boundary it is the only thing that makes
        them match.
        """
        for duplicate, source in self.column_mirrors.get(table_name, {}).items():
            if duplicate in frame.columns and source in frame.columns:
                frame[duplicate] = frame[source].values
        return frame
    ...
    def save(self, directory: str | Path) -> None:
        ...
        manifest = {
            ...
            "column_mirrors": self.column_mirrors,
    ...
    def load(cls, directory: str | Path) -> RelationalSPN:
        ...
        model.column_mirrors = manifest.get("column_mirrors", {})
